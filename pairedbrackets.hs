...
makeNestedParenOfSize n | n <= 0 = PEmpty
makeNestedParenOfSize n = PLeft $ insertRight (makeNestedParenOfSize (n-1))
  where
    insertRight :: Paren n -> Paren (S n)
    insertRight PEmpty     = PRight PEmpty
    insertRight (PRight p) = PRight (PRight p)
    insertRight (PLeft p)  = PLeft (insertRight p)
